package Daje::Database::View::Super::vUsersDocumentation;
use Mojo::Base 'Daje::Database::Model::Super::Common::Base', -base, -signatures, -async_await;
use v5.42;

# NAME
# ====
#
# Daje::Database::View::Super::vUsersDocumentation - Daje db model
#
# SYNOPSIS
# ========
#
#
#       use Daje::Database::View::Super::vUsersDocumentation;
#
#       my $table = Daje::Database::View::Super::vUsersDocumentation->new(db => $db);
#
#       my $result = $table->load_users_documentation_pkey($self, $users_documentation_pkey);
#
#       my $result = $table->load_users_users_fkey($self, $users_users_fkey);
##
#
#
# DESCRIPTION
# ===========
#
# Daje::Database::View::Super::vUsersDocumentation is a View super class
#
# METHODS
# =======
#
#
# LICENSE
# =======
#
# Copyright (C) janeskil1525.
#
# This library is free software; you can redistribute it and/or modify
# it under the same terms as Perl itself.
#
# AUTHOR
# ======
#
# janeskil1525 E<lt>janeskil1525@gmail.com
#

# This file is generated automatically by Daje Tools 2026-04-08 13:48:23.
# It will be re-created by Daje Tools again and any changes
# will be over written.
#
# <!-- Autogenerated file 2026-04-08 13:48:23 -->

our $VERSION = '0.01';

has 'fields' => '"users_documentation_pkey", "editnum", "insby", "insdatetime", "modby", "moddatetime","document_name","users_users_fkey","document_type","document"';
has 'primary_key_name' => "users_documentation_pkey";
has 'table_name' => "v_users_documentation";


async sub load_users_users_fkey_p($self, $companies_pkey, $users_pkey, $users_users_fkey) {
    return $self->load_users_users_fkey($companies_pkey, $users_pkey, $users_users_fkey);
}

sub load_users_users_fkey($self, $companies_pkey, $users_pkey, $users_users_fkey) {
    return $self->load_fkey(
        $self->table_name, $self->fields(), "users_users_fkey", $users_users_fkey
    );
}
async sub load_users_documentation_pkey_p($self, $companies_pkey, $users_pkey, $users_documentation_pkey) {
    return $self->load_users_documentation_pkey($companies_pkey, $users_pkey, $users_documentation_pkey);
}

sub load_users_documentation_pkey($self, $companies_pkey, $users_pkey, $users_documentation_pkey) {
    return $self->load_pk(
        $self->table_name, $self->fields(), $self->primary_key_name(), $users_documentation_pkey
    );
}

1;

