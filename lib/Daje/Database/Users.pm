...
-- Last generated 2026-04-01 15:07:41
...
CREATE OR REPLACE VIEW v_users_users_list AS -- 
    SELECT users_users_pkey, editnum, insby, insdatetime, modby, moddatetime,users_workflow_fkey, password, active, avatar, mail, name, comment, position, biography,      (SELECT name FROM authorities_role WHERE authorities_role_pkey = authorities_role_fkey) as authorities_role_name,authorities_role_fkey, phone, department, employed,      (SELECT iso_code FROM languages_lan WHERE languages_lan_pkey = languages_lan_fkey) as languages_lan_iso_code,languages_lan_fkey, verified, country, city, zipcode, addressline1, region, avatarurl, sendinvitation, addressline2, officelocation, notes     FROM users_users;
