...
sub load_all_users_users($self) {
    ...
    $self->v_users_users_list->load_all_users_users_p($companies_pkey, $users_pkey)->then(sub($result) {
...
sub load_list_users_workflow_fkey($self) {
    ...
    $self->v_users__list->users_workflow_fkey_p($companies_pkey, $users_pkey,$fkey)->then(sub($result) {
        $self->render(json => $result->{data});
    })->catch(sub($err) {
